...
#include "ui_analitikakomintentikorekcija.h"

AnalitikaKomintentiKorekcija::AnalitikaKomintentiKorekcija(BaseForm *parent) :
    BaseForm(parent),
    statusWait(false),
    hlp(0),
    statusOpenEditor(false),
    ui(new Ui::AnalitikaKomintentiKorekcija)
{
    ui->setupUi(this);
    ...
    b = new QBTemplate<AnalitikaKomintentiT>();
    bd = new QBTemplate<AnalitikaKomintentiDetailT>();
...
AnalitikaKomintentiKorekcija::~AnalitikaKomintentiKorekcija()
{
    Singleton *s = Singleton::Instance();
...
void AnalitikaKomintentiKorekcija::pressEscape()
{
    emit signalpressEscape();
}

void AnalitikaKomintentiKorekcija::setFocusArtikal(artikalT t)
{
    ui->artikal->setFocus();
...
void AnalitikaKomintentiKorekcija::setFocusKomintent(komintentT t)
{
...
void AnalitikaKomintentiKorekcija::pressReturn()
{
...
void AnalitikaKomintentiKorekcija::OpenTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    switch (index.column()){
...
void AnalitikaKomintentiKorekcija::CloseTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    table->closePersistentEditor(index);
}


void AnalitikaKomintentiKorekcija::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    m_index = modelX;
    m_row = modelX.row();
}
...
void AnalitikaKomintentiKorekcija::selectionChangedDetail(QModelIndex modelX,QModelIndex modelY)
{
    if (modelX != modelY){
        CloseTablePersistentEditor(ui->tableView, m_index);
    }
...
void AnalitikaKomintentiKorekcija::initProc(AnalitikaKomintenti_trans m_data)
{
...
void AnalitikaKomintentiKorekcija::procDeleteItem(){
    QList<AnalitikaKomintentiDetailT> data = resFakturaItems;
    bd->RemoveItem(data, m_row);
    resFakturaItems = data;
}

bool AnalitikaKomintentiKorekcija::procAddItem(){

    QList<AnalitikaKomintentiDetailT> data = resFakturaItems;
    AnalitikaKomintentiDetailT item;
...
void AnalitikaKomintentiKorekcija::on_pushButton_6_clicked()
{
    PressKeyReturn(this);
}


void AnalitikaKomintentiKorekcija::on_pushButton_3_clicked()
{
    PressKeyReturn(this);
}

void AnalitikaKomintentiKorekcija::updateFont()
{
    Singleton *s = Singleton::Instance();
...
void AnalitikaKomintentiKorekcija::updateStructCellLineEdit(const QModelIndex & index, QString & value)
{
    switch (index.column()){
...
void AnalitikaKomintentiKorekcija::on_pushButton_4_clicked()
{
    // update faktura
    resFaktura.dokument_tip = "20";
    QList<dokumentT> dok;
    QList<AnalitikaKomintentiT> AnalitikaKomintentiList;
    AnalitikaKomintentiList.append(resFaktura);

    b->ConvertAnyToDokument(AnalitikaKomintentiList, dok);
...
bool AnalitikaKomintentiKorekcija::eventFilter(QObject *object, QEvent *event)
{
    Singleton *s = Singleton::Instance();
...
void AnalitikaKomintentiKorekcija::Refresh()
{
    updateFont();
}
