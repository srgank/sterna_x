...
#include "ui_analitikakomintentilista.h"



AnalitikaKomintentiLista::AnalitikaKomintentiLista(BaseForm *parent) :
    BaseForm(parent),
    ui(new Ui::AnalitikaKomintentiLista)
  , enableClose(true)
{
    ui->setupUi(this);
    ...
    bc = new QBTemplate<AnalitikaKomintentiDetailT>();
...
AnalitikaKomintentiLista::~AnalitikaKomintentiLista()
{
...
void AnalitikaKomintentiLista::pressF2()
{
    if (enableClose){
        emit signalpressF2();
    }
}
void AnalitikaKomintentiLista::pressF3()
{
    if (enableClose){
        emit signalpressF3();
    }
}
void AnalitikaKomintentiLista::pressEscape()
{
    if (enableClose){
        emit signalpressEscape();
    }
}
void AnalitikaKomintentiLista::seTableSelectedRow(int m_row)
{
    ui->tableView->selectRow(m_row);
}
QString AnalitikaKomintentiLista::getSearchString()
{
    return ui->prebaruvanje_po_komintent->text();
}

void AnalitikaKomintentiLista::setSearchString(QString& searchText)
{
    ui->prebaruvanje_po_komintent->setText(searchText);
}

void AnalitikaKomintentiLista::on_lineEdit_textChanged(const QString &arg1)
{
    enableClose = false;
    numOffset = 0;
    QString vLimit = "500";
    ...
    QList<AnalitikaKomintentiT> resFaktura;
    b->ConvertDokument(res, resFaktura);
    resFakturaTemp = resFaktura;
    b->ShowData(resFaktura, model, header, ui->tableView, colWidth);
    enableClose = true;
}
void AnalitikaKomintentiLista::procSectionResized(int a, int b, int c)
{
    colWidth[a] = c;
}
void AnalitikaKomintentiLista::procSectionResizedDetail(int a, int b, int c)
{
    colDetailWidth[a] = c;
}
void AnalitikaKomintentiLista::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    enableClose = false;
    disconnect(sm, SIGNAL(currentRowChanged(QModelIndex,QModelIndex)), this, SLOT(selectionChanged(QModelIndex,QModelIndex)));
    int i = modelX.row();
    ...
    QList<AnalitikaKomintentiDetailT> resFakturaDetail;
    ...
    QList<AnalitikaKomintentiDetailT> data = resFakturaDetail;
...
void AnalitikaKomintentiLista::initProc(int searchIDList, QString& searchStrList, int searchOffsetList)
{
...
AnalitikaKomintenti_trans AnalitikaKomintentiLista::getFakturaData(){
    AnalitikaKomintenti_trans temp;
    temp.data1 = currentData;
    temp.data2 = resFakturaDetailTemp;
    return temp;
}
void AnalitikaKomintentiLista::updateFont()
{
    Singleton *s = Singleton::Instance();
...
void AnalitikaKomintentiLista::on_prebaruvanje_po_komintent_textChanged(const QString &arg1)
{
    on_lineEdit_textChanged(arg1);
}

bool AnalitikaKomintentiLista::eventFilter(QObject *object, QEvent *event)
{
    Singleton *s = Singleton::Instance();
...
void AnalitikaKomintentiLista::Refresh()
{
    updateFont();
}
