...
#include "ui_analitikakomintentivnes.h"

AnalitikaKomintentiVnes::AnalitikaKomintentiVnes(BaseForm *parent) :
    BaseForm(parent),
    statusWait(false),
    hlp(0),
    statusOpenEditor(false),
    ui(new Ui::AnalitikaKomintentiVnes)
{
    ui->setupUi(this);
...
AnalitikaKomintentiVnes::~AnalitikaKomintentiVnes()
{
    Singleton *s = Singleton::Instance();
...
void AnalitikaKomintentiVnes::pressEscape()
{
...
void AnalitikaKomintentiVnes::on_pushButton_released()
{
...
void AnalitikaKomintentiVnes::setFocusArtikal(artikalT t)
{
    ui->artikal->setFocus();
...
void AnalitikaKomintentiVnes::setFocusKomintent(komintentT t)
{
...
void AnalitikaKomintentiVnes::pressReturn()
{
...
void AnalitikaKomintentiVnes::OpenTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    switch (index.column()){
...
void AnalitikaKomintentiVnes::CloseTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    table->closePersistentEditor(index);
}

void AnalitikaKomintentiVnes::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    m_index = modelX;
    m_row = modelX.row();
}

void AnalitikaKomintentiVnes::selectionChangedDetail(QModelIndex modelX,QModelIndex modelY)
{
    if (modelX != modelY){
        CloseTablePersistentEditor(ui->tableView, m_index);
    }
...
void AnalitikaKomintentiVnes::updateStructCellLineEdit(const QModelIndex & index, QString & value)
{
    switch (index.column()){
...
void AnalitikaKomintentiVnes::initProc(AnalitikaKomintenti_trans m_data)
{
    resFakturaItems = m_data.data2;
    showData();
}

void AnalitikaKomintentiVnes::procDeleteItem(){
    QList<AnalitikaKomintentiDetailT> data = resFakturaItems;
    bd->RemoveItem(data, m_row);
    resFakturaItems = data;
}

bool AnalitikaKomintentiVnes::procAddItem(){
    QList<AnalitikaKomintentiDetailT> data = resFakturaItems;
...
void AnalitikaKomintentiVnes::showData(){
...
void AnalitikaKomintentiVnes::on_pushButton_6_clicked()
{
    PressKeyReturn(this);
}


void AnalitikaKomintentiVnes::on_pushButton_3_clicked()
{
    PressKeyReturn(this);
}

void AnalitikaKomintentiVnes::updateFont()
{
    Singleton *s = Singleton::Instance();
...
void AnalitikaKomintentiVnes::on_pushButton_4_clicked()
{
...
bool AnalitikaKomintentiVnes::eventFilter(QObject *object, QEvent *event)
{
    Singleton *s = Singleton::Instance();
...
void AnalitikaKomintentiVnes::Refresh()
{
    updateFont();
}
