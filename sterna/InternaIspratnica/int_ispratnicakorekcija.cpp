...
#include "ui_int_ispratnicakorekcija.h"

IntIspratniciKorekcija::IntIspratniciKorekcija(BaseForm *parent) :
    BaseForm(parent),
    statusWait(false),
    hlp(0),
    statusOpenEditor(false),
    ui(new Ui::IntIspratniciKorekcija)
{
    ui->setupUi(this);
...
IntIspratniciKorekcija::~IntIspratniciKorekcija()
{
    Singleton *s = Singleton::Instance();
...
void IntIspratniciKorekcija::pressEscape()
{
    emit signalpressEscape();
}

void IntIspratniciKorekcija::setFocusArtikal(artikalT t)
{
    ui->artikal->setFocus();
...
void IntIspratniciKorekcija::setFocusKomintent(komintentT t)
{
...
void IntIspratniciKorekcija::pressReturn()
{
...
void IntIspratniciKorekcija::OpenTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    switch (index.column()){
...
void IntIspratniciKorekcija::CloseTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    table->closePersistentEditor(index);
}


void IntIspratniciKorekcija::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    m_index = modelX;
    m_row = modelX.row();
}
...
void IntIspratniciKorekcija::selectionChangedDetail(QModelIndex modelX,QModelIndex modelY)
{
    if (modelX != modelY){
        CloseTablePersistentEditor(ui->tableView, m_index);
    }
...
void IntIspratniciKorekcija::initProc(IntIspratnici_trans m_data)
{
...
void IntIspratniciKorekcija::showData(){
    QList<IntIspratniciDetailT> data = resFakturaItems;
...
void IntIspratniciKorekcija::on_pushButton_6_clicked()
{
    PressKeyReturn(this);
}


void IntIspratniciKorekcija::on_pushButton_3_clicked()
{
    PressKeyReturn(this);
}

void IntIspratniciKorekcija::updateFont()
{
    Singleton *s = Singleton::Instance();
...
void IntIspratniciKorekcija::updateStructCellLineEdit(const QModelIndex & index, QString & value)
{
    switch (index.column()){
...
void IntIspratniciKorekcija::on_pushButton_4_clicked()
{
    // update faktura
...
bool IntIspratniciKorekcija::eventFilter(QObject *object, QEvent *event)
{
    Singleton *s = Singleton::Instance();
...
void IntIspratniciKorekcija::Refresh()
{
    updateFont();
}
