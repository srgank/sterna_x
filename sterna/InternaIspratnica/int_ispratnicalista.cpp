...
#include "ui_int_ispratnicalista.h"



IntIspratniciLista::IntIspratniciLista(BaseForm *parent) :
    BaseForm(parent),
    ui(new Ui::IntIspratniciLista)
  , enableClose(true)
{
    ui->setupUi(this);
    ...
    bc = new QBTemplate<IntIspratniciDetailT>();
...
IntIspratniciLista::~IntIspratniciLista()
{
...
void IntIspratniciLista::pressF2()
{
    if (enableClose){
        emit signalpressF2();
    }
}
void IntIspratniciLista::pressF3()
{
    if (enableClose){
        emit signalpressF3();
    }
}
void IntIspratniciLista::pressEscape()
{
    if (enableClose){
        emit signalpressEscape();
    }
}
void IntIspratniciLista::seTableSelectedRow(int m_row)
{
    ui->tableView->selectRow(m_row);
}
QString IntIspratniciLista::getSearchString()
{
    return ui->prebaruvanje_po_komintent->text();
}

void IntIspratniciLista::setSearchString(QString& searchText)
{
    ui->prebaruvanje_po_komintent->setText(searchText);
}

void IntIspratniciLista::on_lineEdit_textChanged(const QString &arg1)
{
    enableClose = false;
    numOffset = 0;
    QString vLimit = "500";
    ...
    QList<IntIspratniciT> resFaktura;
    b->ConvertDokument(res, resFaktura);
    resFakturaTemp = resFaktura;
    b->ShowData(resFaktura, model, header, ui->tableView, colWidth);
    enableClose = true;
}
void IntIspratniciLista::procSectionResized(int a, int b, int c)
{
    colWidth[a] = c;
}
void IntIspratniciLista::procSectionResizedDetail(int a, int b, int c)
{
    colDetailWidth[a] = c;
}
void IntIspratniciLista::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    enableClose = false;
    disconnect(sm, SIGNAL(currentRowChanged(QModelIndex,QModelIndex)), this, SLOT(selectionChanged(QModelIndex,QModelIndex)));
    int i = modelX.row();
    ...
    QList<IntIspratniciDetailT> resFakturaDetail;
    ...
    QList<IntIspratniciDetailT> data = resFakturaDetail;
...
void IntIspratniciLista::initProc(int searchIDList, QString& searchStrList, int searchOffsetList)
{
...
IntIspratnici_trans IntIspratniciLista::getFakturaData(){
    IntIspratnici_trans temp;
    temp.data1 = currentData;
    temp.data2 = resFakturaDetailTemp;
    return temp;
}
void IntIspratniciLista::updateFont()
{
    Singleton *s = Singleton::Instance();
...
void IntIspratniciLista::on_prebaruvanje_po_komintent_textChanged(const QString &arg1)
{
    on_lineEdit_textChanged(arg1);
}

bool IntIspratniciLista::eventFilter(QObject *object, QEvent *event)
{
    Singleton *s = Singleton::Instance();
...
void IntIspratniciLista::Refresh()
{
    updateFont();
}
