...
#include "ui_int_priemnicalista.h"



IntPriemniciLista::IntPriemniciLista(BaseForm *parent) :
    BaseForm(parent),
    ui(new Ui::IntPriemniciLista)
  , enableClose(true)
{
    ui->setupUi(this);
    ...
    bc = new QBTemplate<IntPriemniciDetailT>();
...
IntPriemniciLista::~IntPriemniciLista()
{
...
void IntPriemniciLista::pressF2()
{
    if (enableClose){
        emit signalpressF2();
    }
}
void IntPriemniciLista::pressF3()
{
    if (enableClose){
        emit signalpressF3();
    }
}
void IntPriemniciLista::pressEscape()
{
    if (enableClose){
        emit signalpressEscape();
    }
}
void IntPriemniciLista::seTableSelectedRow(int m_row)
{
    ui->tableView->selectRow(m_row);
}
QString IntPriemniciLista::getSearchString()
{
    return ui->prebaruvanje_po_komintent->text();
}

void IntPriemniciLista::setSearchString(QString& searchText)
{
    ui->prebaruvanje_po_komintent->setText(searchText);
}

void IntPriemniciLista::on_lineEdit_textChanged(const QString &arg1)
{
    enableClose = false;
    numOffset = 0;
    QString vLimit = "500";
    ...
    QList<IntPriemniciT> resFaktura;
    b->ConvertDokument(res, resFaktura);
    resFakturaTemp = resFaktura;
    b->ShowData(resFaktura, model, header, ui->tableView, colWidth);
    enableClose = true;
}
void IntPriemniciLista::procSectionResized(int a, int b, int c)
{
    colWidth[a] = c;
}
void IntPriemniciLista::procSectionResizedDetail(int a, int b, int c)
{
    colDetailWidth[a] = c;
}
void IntPriemniciLista::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    enableClose = false;
    disconnect(sm, SIGNAL(currentRowChanged(QModelIndex,QModelIndex)), this, SLOT(selectionChanged(QModelIndex,QModelIndex)));
    int i = modelX.row();
    ...
    QList<IntPriemniciDetailT> resFakturaDetail;
    ...
    QList<IntPriemniciDetailT> data = resFakturaDetail;
...
void IntPriemniciLista::initProc(int searchIDList, QString& searchStrList, int searchOffsetList)
{
...
IntPriemnici_trans IntPriemniciLista::getFakturaData(){
    IntPriemnici_trans temp;
    temp.data1 = currentData;
    temp.data2 = resFakturaDetailTemp;
    return temp;
}
void IntPriemniciLista::updateFont()
{
    Singleton *s = Singleton::Instance();
...
void IntPriemniciLista::on_prebaruvanje_po_komintent_textChanged(const QString &arg1)
{
    on_lineEdit_textChanged(arg1);
}

bool IntPriemniciLista::eventFilter(QObject *object, QEvent *event)
{
    Singleton *s = Singleton::Instance();
...
void IntPriemniciLista::Refresh()
{
    updateFont();
}
