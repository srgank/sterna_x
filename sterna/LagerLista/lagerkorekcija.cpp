...
#include "ui_lagerkorekcija.h"

LagerKorekcija::LagerKorekcija(BaseForm *parent) :
    BaseForm(parent),
    statusWait(false),
    hlp(0),
    statusOpenEditor(false),
    ui(new Ui::LagerKorekcija)
{
    ui->setupUi(this);
    ...
    b = new QBTemplate<LagerT>();
    bd = new QBTemplate<LagerDetailT>();
...
void LagerKorekcija::pressEscape()
{
    emit signalpressEscape();
}

void LagerKorekcija::setFocusArtikal(artikalT t)
{
    ui->artikal->setFocus();
...
void LagerKorekcija::setFocusKomintent(komintentT t)
{
...
void LagerKorekcija::pressReturn()
{
...
void LagerKorekcija::OpenTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    switch (index.column()){
...
void LagerKorekcija::CloseTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    table->closePersistentEditor(index);
}


void LagerKorekcija::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    m_index = modelX;
    m_row = modelX.row();
}
...
void LagerKorekcija::selectionChangedDetail(QModelIndex modelX,QModelIndex modelY)
{
    if (modelX != modelY){
        CloseTablePersistentEditor(ui->tableView, m_index);
    }
...
void LagerKorekcija::initProc(Lager_trans m_data)
{
...
void LagerKorekcija::procDeleteItem(){
    QList<LagerDetailT> data = resFakturaItems;
    bd->RemoveItem(data, m_row);
    resFakturaItems = data;
}

bool LagerKorekcija::procAddItem(){
...
void LagerKorekcija::on_pushButton_6_clicked()
{
    PressKeyReturn(this);
}


void LagerKorekcija::on_pushButton_3_clicked()
{
    PressKeyReturn(this);
}

void LagerKorekcija::updateFont()
{
    Singleton *s = Singleton::Instance();
...
void LagerKorekcija::updateStructCellLineEdit(const QModelIndex & index, QString & value)
{
    switch (index.column()){
...
void LagerKorekcija::on_pushButton_4_clicked()
{
    // update faktura
    resFaktura.dokument_tip = "20";
    QList<dokumentT> dok;
    QList<LagerT> LagerList;
    LagerList.append(resFaktura);

    b->ConvertAnyToDokument(LagerList, dok);
...
bool LagerKorekcija::eventFilter(QObject *object, QEvent *event)
{
    Singleton *s = Singleton::Instance();
