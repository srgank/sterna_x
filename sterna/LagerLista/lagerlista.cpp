...
#include "ui_lagerlista.h"



LagerLista::LagerLista(BaseForm *parent) :
    BaseForm(parent),
    ui(new Ui::LagerLista)
  , enableClose(true)
{
    ui->setupUi(this);
    ...
    bc = new QBTemplate<LagerDetailT>();
...
void LagerLista::pressF2()
{
    if (enableClose){
        emit signalpressF2();
    }
}
void LagerLista::pressF3()
{
    if (enableClose){
        emit signalpressF3();
    }
}
void LagerLista::pressEscape()
{
    if (enableClose){
        emit signalpressEscape();
    }
}
void LagerLista::seTableSelectedRow(int m_row)
{
    ui->tableView->selectRow(m_row);
}
QString LagerLista::getSearchString()
{
    return ui->prebaruvanje_po_komintent->text();
}

void LagerLista::setSearchString(QString& searchText)
{
    ui->prebaruvanje_po_komintent->setText(searchText);
}

void LagerLista::on_lineEdit_textChanged(const QString &arg1)
{
    enableClose = false;
    numOffset = 0;
    QString vLimit = "500";
    ...
    QList<LagerT> resFaktura;
    b->ConvertDokument(res, resFaktura);
    resFakturaTemp = resFaktura;
    b->ShowData(resFaktura, model, header, ui->tableView, colWidth);
    enableClose = true;
}
void LagerLista::procSectionResized(int a, int b, int c)
{
    colWidth[a] = c;
}
void LagerLista::procSectionResizedDetail(int a, int b, int c)
{
    colDetailWidth[a] = c;
}
void LagerLista::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    enableClose = false;
    disconnect(sm, SIGNAL(currentRowChanged(QModelIndex,QModelIndex)), this, SLOT(selectionChanged(QModelIndex,QModelIndex)));
    int i = modelX.row();
    ...
    QList<LagerDetailT> resFakturaDetail;
    ...
    QList<LagerDetailT> data = resFakturaDetail;
...
void LagerLista::initProc(int searchIDList, QString& searchStrList, int searchOffsetList)
{
...
Lager_trans LagerLista::getLagerData(){
    Lager_trans temp;
    temp.data1 = currentData;
    temp.data2 = resFakturaDetailTemp;
    return temp;
}
void LagerLista::updateFont()
{
    Singleton *s = Singleton::Instance();
...
void LagerLista::on_prebaruvanje_po_komintent_textChanged(const QString &arg1)
{
    on_lineEdit_textChanged(arg1);
}

bool LagerLista::eventFilter(QObject *object, QEvent *event)
{
    Singleton *s = Singleton::Instance();
