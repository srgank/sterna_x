#include "lagervnes.h"
#include "ui_lagervnes.h"

LagerVnes::LagerVnes(BaseForm *parent) :
    BaseForm(parent),
    statusWait(false),
    hlp(0),
...
void LagerVnes::on_pushButton_released()
{
...
void LagerVnes::setFocusArtikal(artikalT t)
{
    ui->artikal->setFocus();
...
void LagerVnes::setFocusKomintent(komintentT t)
{
...
void LagerVnes::OpenTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    switch (index.column()){
...
void LagerVnes::CloseTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    table->closePersistentEditor(index);
}

void LagerVnes::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    m_index = modelX;
    m_row = modelX.row();
}

void LagerVnes::selectionChangedDetail(QModelIndex modelX,QModelIndex modelY)
{
    if (modelX != modelY){
        CloseTablePersistentEditor(ui->tableView, m_index);
    }
...
void LagerVnes::updateStructCellLineEdit(const QModelIndex & index, QString & value)
{
    switch (index.column()){
...
void LagerVnes::on_pushButton_6_clicked()
{
    PressKeyReturn(this);
}


void LagerVnes::on_pushButton_3_clicked()
{
    PressKeyReturn(this);
}

void LagerVnes::updateFont()
{
    Singleton *s = Singleton::Instance();
...
void LagerVnes::on_pushButton_4_clicked()
{
...
bool LagerVnes::eventFilter(QObject *object, QEvent *event)
{
    Singleton *s = Singleton::Instance();
