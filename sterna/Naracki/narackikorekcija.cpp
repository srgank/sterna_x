    ...
    ui(new Ui::NarackiKorekcija)
{
    ...
    bd = new QBTemplate<NarackiDetailT>();
...
void NarackiKorekcija::setFocusArtikal(artikalT t)
{
    ui->artikal->setFocus();
...
void NarackiKorekcija::setFocusKomintent(komintentT t)
{
...
void NarackiKorekcija::OpenTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    switch (index.column()){
...
void NarackiKorekcija::CloseTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    table->closePersistentEditor(index);
}


void NarackiKorekcija::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    m_index = modelX;
    m_row = modelX.row();
}
...
void NarackiKorekcija::selectionChangedDetail(QModelIndex modelX,QModelIndex modelY)
{
    if (modelX != modelY){
        CloseTablePersistentEditor(ui->tableView, m_index);
    }
...
void NarackiKorekcija::initProc(Naracki_trans m_data)
{
...
void NarackiKorekcija::procDeleteItem(){
    QList<NarackiDetailT> data = resFakturaItems;
    bd->RemoveItem(data, m_row);
    resFakturaItems = data;
}

bool NarackiKorekcija::procAddItem(){
...
void NarackiKorekcija::showData(){
    QList<NarackiDetailT> data = resFakturaItems;
...
void NarackiKorekcija::on_pushButton_6_clicked()
{
    PressKeyReturn(this);
}


void NarackiKorekcija::on_pushButton_3_clicked()
{
    PressKeyReturn(this);
}

void NarackiKorekcija::updateFont()
{
    Singleton *s = Singleton::Instance();
...
void NarackiKorekcija::updateStructCellLineEdit(const QModelIndex & index, QString & value)
{
    switch (index.column()){
...
void NarackiKorekcija::on_pushButton_4_clicked()
{
    // update faktura
    resFaktura.dokument_tip = "20";
    QList<dokumentT> dok;
    QList<NarackiT> NarackiList;
    NarackiList.append(resFaktura);

    b->ConvertAnyToDokument(NarackiList, dok);
...
bool NarackiKorekcija::eventFilter(QObject *object, QEvent *event)
{
    Singleton *s = Singleton::Instance();
