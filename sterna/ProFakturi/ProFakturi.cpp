...
{
    if (m_ProFakturiLista){
        m_data = m_ProFakturiLista->getFakturaData();
    }else{
        return;
    }
    searchIDList = m_ProFakturiLista->geTableSelectedRow();
...
void ProFakturi::Refresh()
{
    if (m_ProFakturiLista){
        PressKeyF12(m_ProFakturiLista);
    }else if(m_ProFakturiVnes){
        PressKeyF12(m_ProFakturiVnes);
    }else if(m_ProFakturiKorekcija){
        PressKeyF12(m_ProFakturiKorekcija);
    }else{
    }
}

ProFaktura_trans& ProFakturi::getFaktTransData(){
    if (m_ProFakturiLista){
        m_data = m_ProFakturiLista->getFakturaData();
    }else{
    }
