    ...
    ui(new Ui::SmetkiKorekcija)
{
    ...
    bd = new QBTemplate<SmetkiDetailT>();
...
void SmetkiKorekcija::setFocusKomintent(komintentT t)
{
...
void SmetkiKorekcija::OpenTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    switch (index.column()){
...
void SmetkiKorekcija::CloseTablePersistentEditor(QTableView * table, QModelIndex &index)
{
    table->closePersistentEditor(index);
}


void SmetkiKorekcija::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    m_index = modelX;
    m_row = modelX.row();
}
...
void SmetkiKorekcija::selectionChangedDetail(QModelIndex modelX,QModelIndex modelY)
{
    if (modelX != modelY){
        CloseTablePersistentEditor(ui->tableView, m_index);
    }
...
void SmetkiKorekcija::initProc(Smetki_trans m_data)
{
...
void SmetkiKorekcija::procDeleteItem(){
    QList<SmetkiDetailT> data = resFakturaItems;
    bd->RemoveItem(data, m_row);
    resFakturaItems = data;
}

bool SmetkiKorekcija::procAddItem(){
...
void SmetkiKorekcija::on_pushButton_6_clicked()
{
    PressKeyReturn(this);
}


void SmetkiKorekcija::on_pushButton_3_clicked()
{
    PressKeyReturn(this);
}

void SmetkiKorekcija::updateFont()
{
    Singleton *s = Singleton::Instance();
...
void SmetkiKorekcija::updateStructCellLineEdit(const QModelIndex & index, QString & value)
{
    switch (index.column()){
...
void SmetkiKorekcija::on_pushButton_4_clicked()
{
    // update faktura
    resFaktura.dokument_tip = "20";
    QList<dokumentT> dok;
    QList<SmetkiT> SmetkiList;
    SmetkiList.append(resFaktura);

    b->ConvertAnyToDokument(SmetkiList, dok);
...
bool SmetkiKorekcija::eventFilter(QObject *object, QEvent *event)
{
    Singleton *s = Singleton::Instance();
